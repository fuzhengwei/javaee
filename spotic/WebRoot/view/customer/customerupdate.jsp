  ...
  <head>
    ...
    <link href="res/EasyValidator3.0/EasyValidator/css/validate.css" rel="stylesheet" type="text/css" />
 	<script type="text/javascript">
		...
		$(function(){
			$(".customer_type option[value='${model.customer_type}']").attr("selected","selected");
		});
	</script>
  </head>
  
  <body>
  <form action="Spotic_CustomerServlet?type=customerupdate&customer_id=${ model.customer_id }" method="post">
    <table border="1">
	<tr>
		<td style="background-color: blue;color: white;font-weight: bolder;"><fmt:message key="spotic_customer.customer_company"/><%--客户公司：--%></td>
		<td><input type="text" name="customer_company" value="${ model.customer_company }" reg="^.{2,20}$" tip="客户公司[3到20个长度] 如：百度"/><span style="color: red;margin-left: 5px;">*</span></td>
	</tr>
	<tr>
		<td style="background-color: blue;color: white;font-weight: bolder;"><fmt:message key="spotic_customer.customer_name"/><%--客户姓名：--%></td>
		<td><input type="text" name="customer_name" value="${ model.customer_name }" reg="^[a-zA-Z]{3,20}$" tip="客户姓名[3到20个英文字母] 如: generalfu"/><span style="color: red;margin-left: 5px;">*</span></td>
	</tr>
	<tr>
		<td style="background-color: blue;color: white;font-weight: bolder;"><fmt:message key="spotic_customer.customer_phone"/><%--客户电话：--%></td>
		<td><input type="text" name="customer_phone" value="${ model.customer_phone }" reg="^[1][358]\d{9}$" tip="客户电话[中国电话] 如: 13689890001"/><span style="color: red;margin-left: 5px;">*</span></td>
	</tr>
	<tr>
		<td style="background-color: blue;color: white;font-weight: bolder;"><fmt:message key="spotic_customer.customer_fax"/><%--客户传真：--%></td>
		<td><input type="text" name="customer_fax" value="${ model.customer_fax }" reg="^\d{3}-\d{8}$|^\d{4}-\d{7}$" tip="客户传真 如：010-87888822"/><span style="color: red;margin-left: 5px;">*</span></td>
	</tr>
	<tr>
		<td style="background-color: blue;color: white;font-weight: bolder;"><fmt:message key="spotic_customer.customer_email"/><%--客户邮箱：--%></td>
		<td><input type="text" name="customer_email" value="${ model.customer_email }" reg="^\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$" tip="客户邮箱，如generalfu@drdg.com"/><span style="color: red;margin-left: 5px;">*</span></td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td>
			<textarea cols="50" rows="10" name="customer_remarks" onkeyup="return ismaxlength(this)" maxlength="20">${ model.customer_remarks }</textarea>
		</td>
	</tr>
	<tr>
		<td colspan="2">
