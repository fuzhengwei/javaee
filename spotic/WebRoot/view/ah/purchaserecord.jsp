...
<html>
  <head>
    ...
    <title>采购记录 Purchase records</title>
  ...
  <form action="AhServlet?type=eiselectserach&inventory_type=${ inventory_type }" method="post">
	...
	<table border="1" class="searchTable" style="font-size: 12px;">
	 ...
	 <c:if test="${fn:length(requestScope.modelList) == 0}">
  	...
  	<c:if test="${fn:length(requestScope.modelList) != 0}">
 	<table border="1" style="font-size: 12px;"  style="font-size: 12px;" width="100%">
 		...
 		<c:forEach items="${requestScope.modelList}" var="key" varStatus="id" begin="0">
			...
			<td colspan="9" align="right">
				<c:if test="${page > 1}">
					<a href="AhServlet?type=3&page=${ page <= 1?1:page-1 }&inventory_type=${ inventory_type }"><fmt:message key="jsp.onpage"/></a>
					|
				</c:if>
				<a href="AhServlet?type=3&page=${ page + 1 }&inventory_type=${ inventory_type }"><fmt:message key="jsp.nextpage"/></a>
			</td>
		</tr>
 	</table>
 	</c:if>
