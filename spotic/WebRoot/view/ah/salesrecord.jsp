...
<html>
  <head>
    ...
    <title>销售记录 The sales record</title>
  ...
  <form action="AhServlet?type=eiselectserach&inventory_type=${ inventory_type }" method="post">
	...
	<table border="1" class="searchTable" style="font-size: 12px;">
	 ...
	 <c:if test="${fn:length(requestScope.modelList) == 0}">
  	...
  	<c:if test="${fn:length(requestScope.modelList) != 0}">
 	<table border="1" style="font-size: 14px;"  style="font-size: 12px;" width="100%">
		...
		<tr>
			<td colspan="9" align="right">
				<c:if test="${page > 1}">
					<a href="AhServlet?type=1&page=${ page <= 1?1:page-1 }&inventory_type=${ inventory_type }"><fmt:message key="jsp.onpage"/></a>
					|
				</c:if>
				<a href="AhServlet?type=1&page=${ page + 1 }&inventory_type=${ inventory_type }"><fmt:message key="jsp.nextpage"/></a>
			</td>
		</tr>
 	</table>
 	</c:if>
