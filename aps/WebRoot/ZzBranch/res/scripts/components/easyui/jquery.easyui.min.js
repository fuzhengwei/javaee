...
if(_12){
//==gege modified 保证回复到正确的位置
//_12.animate({left:e.data.startLeft,top:e.data.startTop},function(){
_12.animate({left:$(e.data.target).offset().left,top:$(e.data.target).offset().top},function(){
_13.onProxyRevertEnd.call(e.data.target);
//end
_15();
});
}else{
...
//==gege add one callback function
},onProxyRevertEnd:function(){
}};
//end
})(jQuery);
(function($){
...
var _1f0="auto";
//==agang 当fit=false时，_1f1和_1f0变量不存在，因此将For循环代码放入if(opts.height>0){
if(opts.height>0){
...
//==agang增加了一个padding样式，这样所有的Tab效果统一
var pp=$("<div style='padding:5px;'></div>").appendTo($(">div.tabs-panels",_252));
//==agang结束
tabs.push(pp);
_24a(_252,pp,_253);
...
//==gege 增加判断是否是分页返回数据
if(data.rows != undefined && data.rows != null) data=data.rows;
//==end
_557(data,_551);
...
if(opts.pagination){
	if(opts.url.indexOf('?') > -1)
		opts.url = opts.url + '&page=' + opts.pageNumber + '&rows=' + opts.pageSize;
	else
		opts.url = opts.url + '?page=' + opts.pageNumber + '&rows=' + opts.pageSize;
}
//==end
...
if(opts.onLoadSuccess){
	opts.onLoadSuccess.apply(_56e, [row, data, opts])
}
//==end
},error:function(){
_574.removeClass("tree-loading");
...
//==agang 根据data={}方式加载combotree数据，渲染Data
if(opts.data) {
	tree.tree('loadData', opts.data);
}
//==agang结束
};
function _645(_646){
...
$.extend(_64e.options,_64b);
	...
	//==agang 根据data={}方式加载combotree数据，传入data
	_64e = $.data(this,"combotree",{options:$.extend({},$.fn.combotree.defaults,$.fn.combotree.parseOptions(this),_64b,{data:eval($(this).attr('data'))})});
	//==agang结束
}
_63f(this);
});
