...
<html>
  <head>
    <base href="<%=basePath%>">
    <title>inventory for update</title>
    ...
    <script type="text/javascript">
		...
		$(function(){
			$(".fk_customer_id option[value='${model.fk_customer_id}']").attr("selected","selected");
		});
	</script>
  </head>
  <body>
  <form action="Spotic_InventoryServlet?type=inventoryupdate&inventory_id=${ model.inventory_id }" method="post">
  <table border="1">
	<tr>
		<td style="background-color: blue;color: white;font-weight: bolder;"><fmt:message key="spotic_inventory.fk_customer_id"/><%--客户：--%></td>
		<td>
			<select name="fk_customer_id" class="fk_customer_id">
	...
	<tr>
		...
		<td><input type="text" name="inventory_part_no" value="${ model.inventory_part_no }" maxlength="30" reg="^[a-zA-Z0-9]{1,30}$" tip="物料号[30以内的字母数字组合] 如: P1009"/><span style="color: red;margin-left: 5px;">*</span></td>
	</tr>
	<tr>
		<td style="background-color: blue;color: white;font-weight: bolder;"><fmt:message key="spotic_inventory.inventory_mfg"/><%--物料产地：--%></td>
		<td><input type="text" name="inventory_mfg" value="${ model.inventory_mfg }" maxlength="50" reg="^.{1,50}$" tip="物料产地[50个长度以内的组合] 如: 北京1号D001生产院"/><span style="color: red;margin-left: 5px;">*</span>
	</tr>
	<tr>
		<td style="background-color: blue;color: white;font-weight: bolder;"><fmt:message key="spotic_inventory.inventory_dc"/><%--物料年份：--%></td>
		<td><input type="text" name="inventory_dc" value="${ model.inventory_dc }" maxlength="4" reg="^[0-9]{1,4}$" tip="物料年份[4个数字以内] 如：12"/><span style="color: red;margin-left: 5px;">*</span>
	</tr>
	<tr>
		<td style="background-color: blue;color: white;font-weight: bolder;"><fmt:message key="spotic_inventory.inventory_cost"/><%--成本价：--%></td>
		<td><input type="text" name="inventory_cost" value="${ model.inventory_cost }" maxlength="11" reg="^[0-9]{1,11}$" tip="成本价[11个数字以内] 1100"/><span style="color: red;margin-left: 5px;">*</span>
	</tr>
	<tr>
		<td style="background-color: blue;color: white;font-weight: bolder;"><fmt:message key="spotic_inventory.inventory_loc"/><%--产地：--%></td>
		<td><input type="text" name="inventory_loc" value="${ model.inventory_loc }" maxlength="50" reg="^.{1,50}$" tip="产地[50个长度以内的组合] 如: 北京1号D001生产院"/><span style="color: red;margin-left: 5px;">*</span>
	</tr>
	<tr>
		<td style="background-color: blue;color: white;font-weight: bolder;"><fmt:message key="spotic_inventory.inventory_lt"/><%--指定到货周期LT：--%></td>
		<td><input type="text" name="inventory_lt" value="${ model.inventory_lt }" maxlength="20" reg="^.{1,20}$" tip="指定到货周期[20个长度以内的组合] "/><span style="color: red;margin-left: 5px;">*</span>
	</tr>
	<tr>
		<td style="background-color: blue;color: white;font-weight: bolder;"><fmt:message key="spotic_inventory.inventory_type"/><%--需求类型：--%></td>
		<td><input type="hidden" value="${model.inventory_type}" name="inventory_type"/>
			<c:choose>
				<c:when test="${model.inventory_type  == 1}">inventory<%--本公司库存--%></c:when>
				<c:when test="${model.inventory_type  == 2}">customer<%--客户--%></c:when>
				<c:when test="${model.inventory_type  == 3}">excess<%--供应商--%></c:when>
				<c:when test="${model.inventory_type  == 4}">outside<%--市场--%></c:when>
			</c:choose>
		</td>
	</tr>
	<tr>
		...
		<td>
			<c:choose>   
				<c:when test="${model.inventory_type  == 1}">
				...
				<c:when test="${model.inventory_type  == 3}">
					<select name="inventory_state">
				...
				<c:when test="${model.inventory_type  == 4}">
					<select name="inventory_state">
		...
		<td>
			<c:choose>
				<c:when test="${model.inventory_allto_state  == 0}">no assigned<%--
					未分配--%><input type="hidden" name="inventory_allto_state" value="0"/>
				</c:when>
				<c:when test="${model.inventory_allto_state  == 1}">assigned<%--
					已分配--%><input type="hidden" name="inventory_allto_state" value="1"/>
				</c:when>
			</c:choose>
		</td>
	</tr>
	<tr>
		...
		<td>
			<textarea rows="10" cols="30" name="inventory_remarks"  onkeyup="return ismaxlength(this)" maxlength="40">${ model.inventory_remarks }</textarea>
		</td>
	</tr>
	<tr>
		<td colspan="2">
